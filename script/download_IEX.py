...
def get_dates():
    ...
    IEX_date = date_today - timedelta(days = 120) # 30 trailing calendar days acc. to IEX docu
    print(IEX_date,'iex_date')
    ...
    try:
        ...
        start_date = max(IEX_date, last_date + timedelta(days=120))
    except:
        start_date = IEX_date
...
def get_tickers(path):
    ...
    tickers=[]
    # tickers_string = df[~df['isEnabled'] == False].loc[, 'symbol']
    tickers.append('SPY')
    print(tickers)
    return(tickers)
